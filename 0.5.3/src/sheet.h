...
#include "gcode.h"
 ...
 ///@todo: move  to an independant headre file
  ...
  ///the main window GUI class (Hand coded)
...
public:
	 ...
	 void writeSettings();
	 /// @todo: has it to be within windows class?
	 /// Open the specified file and pass it to the file format handler (DXF) for parsing
   ...
   private:
	 ...
	 QAction *stepAction;
         /// Zoom in Scenes to show a rct drawn by the use
         QAction *zoomRectAction;
	 ...
	 QAction *zoomOutAction;
          ...
          /// Zoom in previewcen to fit contents
         QAction *zoomPrevFitAction;
          /// Zoom in previewcen
         QAction *zoomPrevInAction;
          /// Zoom out previewcen
         QAction *zoomPrevOutAction;
	 ...
	 //QAction *mirrorAction;
	  ...
	  //***********************Dock menu ACTIONS************* 
	 ///Show/hide previewDock
	 QAction *previewDockAction;
	 ///Show/hide dockInsertedParts
	 QAction *insertedDockAction;
	 ///Show/hide filesListDock
	...
	/// file toolbar
	 ...
	 QProgressBar *loadingBar;
         /// A label where to show part info and warnings
         QLabel *partInfos;
	 //***********************MODEL************* 
	 ...
	 unsigned int nbrParts;
        /// Affect animation and G-code saving options
         bool plasmaMode;
         /// Show  error when enbcountring open loops
         bool openLoopError;
         /// Show  error when enbcountring duplicate entities
         bool dupliError;

	 /// @note have to stay public 
	 public slots:
	 /// For opening a file via a dialog instead of the parts explorer
	 void openFile();
	...
	void updateMoveVal(double val=20);
	 ...
	 void insert();
	 bool confirmAction();
         void updateRectDims(bool rem=true);
	 protected:
	 virtual void closeEvent(QCloseEvent *event);
  };

	  /// The model of inserted parts list view
